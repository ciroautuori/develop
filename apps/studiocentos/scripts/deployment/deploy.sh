#!/bin/bash
# ============================================================================
# DEPLOY DOCKER PRODUZIONE - StudiocentOS.it
# ============================================================================
...
echo -e "${GREEN}🐳 DEPLOY DOCKER PRODUZIONE - STUDIOCENTOS.IT${NC}"
echo "=================================================="
echo ""

# Directory di lavoro
WORKSPACE_DIR="/home/autcir_gmail_com/studiocentos_ws"
...
# Verifica DNS
echo "🔍 Verifica DNS per studiocentos.it..."
if ! host studiocentos.it &> /dev/null; then
    echo -e "${YELLOW}⚠️  studiocentos.it non risolve!${NC}"
...
timeout 60 bash -c 'until docker compose -f docker-compose.production.yml exec -T postgres pg_isready -U studiocentos > /dev/null 2>&1; do sleep 2; done' || {
...
echo "  1. Verifica che studiocentos.it punti all'IP del server"
...
if [[ $REPLY =~ ^[Yy]$ ]]; then
    ...
    if [ $? -eq 0 ]; then
        ...
        echo "  docker compose -f $DOCKER_DIR/docker-compose.production.yml run --rm certbot certonly --webroot --webroot-path=/var/www/certbot -d studiocentos.it -d www.studiocentos.it"
    fi
else
...
echo ""
echo "🌐 Accesso:"
echo -e "  - ${BLUE}https://studiocentos.it${NC} (Frontend)"
echo -e "  - ${BLUE}https://studiocentos.it/api${NC} (Backend API)"
echo -e "  - ${BLUE}https://studiocentos.it/ai${NC} (AI Service)"
echo ""
echo "📝 Comandi utili:"
