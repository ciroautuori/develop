#!/bin/bash
# ============================================================================
# AVVIO SERVIZI PRODUZIONE - StudiocentOS
# ============================================================================
...
echo -e "${GREEN}🚀 AVVIO SERVIZI PRODUZIONE - STUDIOCENTOS${NC}"
echo "=============================================="
echo ""
...
timeout 30 bash -c 'until docker compose exec -T postgres pg_isready -U studiocentos > /dev/null 2>&1; do sleep 1; done' || {
...
nohup uv run uvicorn app.main:app \
    ...
    --workers 4 \
    --log-level info \
    > /var/log/studiocentos-backend.log 2>&1 &
...
nohup uv run uvicorn app.main:app \
    ...
    --workers 2 \
    --log-level info \
    > /var/log/studiocentos-ai.log 2>&1 &
...
nohup pnpm preview --host 0.0.0.0 --port 3000 \
    > /var/log/studiocentos-frontend.log 2>&1 &
...
echo -e "  - ${BLUE}https://studiocentos.it${NC}"
echo -e "  - ${BLUE}https://studiocentos.it/api${NC} (Backend)"
echo -e "  - ${BLUE}https://studiocentos.it/ai${NC} (AI Service)"
echo ""
echo "📝 Logs:"
echo "  - Backend: tail -f /var/log/studiocentos-backend.log"
echo "  - AI Service: tail -f /var/log/studiocentos-ai.log"
echo "  - Frontend: tail -f /var/log/studiocentos-frontend.log"
echo "  - Nginx: sudo tail -f /var/log/nginx/studiocentos-access.log"
echo ""
echo "🛑 Stop servizi:"
