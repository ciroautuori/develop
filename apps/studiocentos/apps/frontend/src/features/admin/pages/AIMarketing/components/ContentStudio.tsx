...
// API e Costanti - SINGLE SOURCE OF TRUTH
...
// Centralized constants - NO DUPLICATES
import { SOCIAL_PLATFORMS } from '../constants/platforms';
import { SOCIAL_TONES, TONE_PROMPTS } from '../constants/tones';
...
// PLATFORMS - Usa SOCIAL_PLATFORMS dalla costante centralizzata (importata sopra)
// Alias locale per compatibilità
const PLATFORMS = SOCIAL_PLATFORMS;

// TONES - Usa SOCIAL_TONES dalla costante centralizzata (importata sopra)
// Alias locale per compatibilità
const TONES = SOCIAL_TONES;

// QUICK_TEMPLATES - Usa SOCIAL_QUICK_TEMPLATES dalla costante centralizzata (importata sopra)
// Mappa i template per compatibilità con il formato locale usato nel componente
const QUICK_TEMPLATES = SOCIAL_QUICK_TEMPLATES.map(t => ({
  id: t.id.replace('_', '-'), // ID display con trattino per UI
  postType: t.postType,        // ⚠️ ORIGINALE con underscore per backend PRO endpoint!
  label: `${t.icon} ${t.label.replace(t.icon, '').trim()}`,
  textPrompt: t.aiPrompt,
  imageStyle: '', // Generato dinamicamente
  mood: '', // Generato dinamicamente
  // Keep extra data for rich content
  hashtags: t.hashtags,
      ...
      for (const platformId of selectedPlatforms) {
        ...
        const response = await fetch('/api/v1/copilot/marketing/generate', {
          ...
          body: JSON.stringify({
            ...
            type: contentType,
            subtype: contentType,
            // ⚠️ USARE postType ORIGINALE (con underscore) per attivare endpoint PRO!
            post_type: selectedTemplate?.postType || contentType,
            platform: platformId,
            tone,
            length: 'medium',
            ...
            platform_format_instructions: getPlatformSpecificPrompt(platformId),
          }),
        });
...
`.trim();
        ...
        const response = await fetch('/api/v1/copilot/image/generate', {
          ...
          body: JSON.stringify({
            ...
            post_type: selectedTemplate?.postType || contentType,
            sector: brandContext?.industry || 'tech',
            // Pass Brand DNA settings for dynamic branding
            logo_url: brandSettings?.logo_url || null,
            brand_name: brandSettings?.company_name || 'StudioCentOS',
          }),
        });
