...
import {
  ...
  Globe, Search, Activity, TrendingUp
} from 'lucide-react';
...
const PowerContentGenerator = lazy(() => import('./components/PowerContentGenerator'));
...
type ContentSubTab = 'power' | 'templates' | 'video' | 'social' | 'email';
...
const TABS: Tab[] = [
  ...
  },
  {
    ...
    icon: Sparkles,
    description: 'Genera contenuti ottimizzati per Instagram, Facebook, TikTok',
  ...
  },
  {
    ...
    color: 'from-primary via-primary/80 to-primary/70',
  },
  {
    id: 'settings',
    label: 'Impostazioni',
...
const CONTENT_SUB_TABS = [
  { id: 'power' as ContentSubTab, label: '🚀 Power Creator', icon: Sparkles },
...
export function AIMarketing() {
  ...
  const [contentSubTab, setContentSubTab] = useState<ContentSubTab>('power');
  ...
  const handleMultiPlatformClick = () => {
  ...
  const renderDashboard = () => (
    <div className="space-y-6">
      ...
      <motion.div
        ...
        className="p-6 rounded-2xl border bg-gradient-to-r from-primary/10 to-primary/5 border-primary/20"
      >
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h3 className="font-semibold text-lg text-foreground">Multi-Platform Wizard 🚀</h3>
              <p className="text-sm text-muted-foreground">Pubblica su tutti i social in un click</p>
            </div>
          </div>
          <Button
            onClick={handleMultiPlatformClick}
            size="sm"
  ...
  const renderContentTab = () => (
    <div>
      {renderSubTabNav(CONTENT_SUB_TABS, contentSubTab, (tab) => setContentSubTab(tab as ContentSubTab))}
      <Suspense fallback={<TabLoader />}>
        <AnimatePresence mode="wait">
          {contentSubTab === 'power' && (
            <motion.div key="power" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }}>
              <PowerContentGenerator />
            </motion.div>
          )}
