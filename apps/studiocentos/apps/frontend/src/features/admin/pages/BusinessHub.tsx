...
const QUOTE_STATUS = {
  ...
  sent: { label: 'Inviato', color: 'bg-primary', icon: Send },
...
const APPOINTMENT_STATUS = {
  ...
  completed: { label: 'Completato', color: 'bg-primary' },
  cancelled: { label: 'Annullato', color: 'bg-red-500' },
};

const PIPELINE_STAGES = [
  { id: 'lead', label: 'Lead', color: 'bg-gray-500' },
  { id: 'qualified', label: 'Qualificato', color: 'bg-primary' },
  { id: 'proposal', label: 'Proposta', color: 'bg-primary/80' },
...
export function BusinessHub() {
  const [activeTab, setActiveTab] = useState<'customers' | 'quotes' | 'appointments' | 'pipeline' | 'newsletter'>('customers');
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const newsletterRes = await fetch('/api/v1/newsletter/subscribers', { headers });
      if (newsletterRes.ok) {
  ...
  // Tab configuration
  const TABS = [
    { id: 'customers' as const, label: 'Clienti', icon: Users, description: 'Gestione anagrafica clienti', color: 'from-primary to-primary/60' },
    { id: 'quotes' as const, label: 'Preventivi', icon: FileText, description: 'Preventivi e offerte commerciali', color: 'from-primary via-primary/80 to-primary/60' },
    ...
    { id: 'newsletter' as const, label: 'Newsletter', icon: Mail, description: 'Iscritti e lead magnet', color: 'from-purple-500 to-pink-500' },
  ];
      ...
      {/* Newsletter Tab Content */}
      {
        activeTab === 'newsletter' && (
          <motion.div
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            className={`${cardBg} rounded-xl sm:rounded-2xl overflow-hidden`}
          >
            <div className="p-4 sm:p-6 border-b border-border">
              <h3 className="text-lg font-semibold text-foreground">Iscritti Newsletter</h3>
              <p className="text-sm text-muted-foreground">Totale iscritti: {subscribers.length}</p>
            </div>
              ...
              <table className="w-full">
                <thead className={isDark ? 'bg-white/5' : 'bg-gray-50'}>
                  <tr>
                    ...
                    <th className="text-left p-4 text-sm font-medium text-muted-foreground">Data Iscrizione</th>
                  </tr>
                </thead>
                <tbody className={`divide-y ${isDark ? 'divide-white/10' : 'divide-gray-100'}`}>
                  {subscribers.map((sub) => (
                    <tr key={sub.id} className={isDark ? 'hover:bg-white/5' : 'hover:bg-gray-50'}>
                      <td className="p-4 text-sm font-medium text-foreground">{sub.email}</td>
                      <td className="p-4 text-sm text-muted-foreground">{sub.source}</td>
                      <td className="p-4">
                        <span className={`px-2 py-1 rounded-full text-xs font-medium ${sub.status === 'active' ? 'bg-green-500/10 text-green-500' : 'bg-red-500/10 text-red-500'
                          }`}>
                  ...
                  <div className="flex items-center justify-between sm:justify-end gap-2 sm:gap-4">
                    ...
                    <span className={cn(
                      ...
                      customer.status === 'active' ? 'bg-green-500/20 text-green-500' :
                        customer.status === 'lead' ? 'bg-primary/20 text-primary' :
