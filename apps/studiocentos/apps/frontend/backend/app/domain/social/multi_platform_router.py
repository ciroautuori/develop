...
class MultiPlatformPostRequest(BaseModel):
    ...
    platform_prompts: Optional[Dict[str, str]] = Field(None, description="Platform-specific prompts")

    # Publishing
    publish_immediately: bool = Field(default=False, description="Publish now or save as draft")
    schedule_time: Optional[datetime] = Field(None, description="Schedule for future")
