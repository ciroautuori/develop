...
from app.domain.marketing.models import Lead, LeadStatus, EmailCampaign
...
async def launch_acquisition_campaign(
    ...
    try:
        for lead_data in request.leads:
            # 1. Save lead to CRM
            try:
                # Check by email (unique constraint) since place_id doesn't exist in model
                existing = db.execute(
                    text("SELECT id FROM leads WHERE email = :email"),
                    {"email": lead_data.email}
                ).fetchone() if lead_data.email else None
...
async def get_acquisition_pipeline(
    ...
    try:
        ...
        for status, label in stages:
            ...
            # Get sample leads for this stage - use empty list as fallback
            leads_list = []
            try:
...
class AutoPilotRequest(BaseModel):
    ...
    add_to_campaign: Optional[str] = Field(default=None, description="Campaign name to add high-score leads")
