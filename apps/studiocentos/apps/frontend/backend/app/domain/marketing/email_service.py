...
class EmailMessage:
    ...
    from_email: str = "noreply@studiocentos.it"
...
class EmailMarketingService:
    ...
    def __init__(self):
        ...
        self.tracking_domain = getattr(settings, 'EMAIL_TRACKING_DOMAIN', 'https://studiocentos.it')

    def _detect_provider(self) -> EmailProvider:
        """Detect which email provider is configured."""
        if getattr(settings, 'SENDGRID_API_KEY', None):
            return EmailProvider.SENDGRID
        elif getattr(settings, 'MAILGUN_API_KEY', None):
    ...
    async def send(self, message: EmailMessage) -> List[SendResult]:
        ...
        elif self.provider == EmailProvider.MAILGUN and self.mailgun_api_key:
...
class SequenceStep:
    ...
    condition: Optional[str] = None  # "opened", "clicked", "not_opened", "not_clicked"
    next_step_if_true: Optional[int] = None
    next_step_if_false: Optional[int] = None
    new_status: Optional[str] = None  # For UPDATE_LEAD_STATUS
    notification_message: Optional[str] = None  # For NOTIFY_ADMIN
    ...
    async def _process_step(
        ...
        if action == "send_email" and condition_met:
            ...
            if lead and lead[0]:
                ...
                subject = step_config.get("subject", "Follow-up from StudioCentOS")
