...
router = APIRouter(
    ...
    # dependencies=[Depends(get_current_admin_user)]  # 🔐 TEMPORANEAMENTE DISABILITATO PER DEBUG
)
...
def get_calendar_stats(
    ...
    try:
        ...
        # Con native_enum=False e Column(String), row[0] è SEMPRE una stringa
        status_counts = {}
        for row in status_results:
            key = row[0]
            if hasattr(key, 'value'):
                # Fallback se per qualche motivo arriva ancora un Enum
                key = key.value
        ...
        sched_val = PostStatus.SCHEDULED.value if hasattr(PostStatus.SCHEDULED, 'value') else PostStatus.SCHEDULED

        upcoming_week = db.query(func.count(ScheduledPost.id)).filter(
            and_(
                ScheduledPost.scheduled_at >= now,
                ScheduledPost.scheduled_at <= next_week,
                ScheduledPost.status == sched_val
            )
        ).scalar() or 0
        ...
        pub_val = PostStatus.PUBLISHED.value if hasattr(PostStatus.PUBLISHED, 'value') else PostStatus.PUBLISHED
        ...
        import traceback
        structlog.get_logger().error("get_calendar_stats_error", error=str(e), traceback=traceback.format_exc())
        ...
        for i, post_date in enumerate(posting_dates):
            ...
            try:
                ...
                if response.status_code == 200:
                    ...
                    image_url = None
                    try:
                        # Brand identity context - COLORI REALI STUDIOCENTOS (NO BLU!)
                        BRAND_STYLE = "StudioCentos brand, GOLD (#D4AF37) and BLACK (#0A0A0A) colors, premium luxury aesthetic, modern tech Italian excellence, NO BLUE"
