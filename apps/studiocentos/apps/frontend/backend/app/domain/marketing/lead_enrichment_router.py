...
class PlacesSearchRequest(BaseModel):
    ...
    location: Optional[str] = Field(None, description="Città/regione (es. 'Salerno, Italia')")
    radius_km: int = Field(50, ge=5, le=200, description="Raggio ricerca in km")
    industry: Optional[str] = Field(None, description="Settore per filtraggio")
...
async def enrich_lead(
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"Errore arricchimento: {str(e)}"
        )
    finally:
        await service.close()
...
async def bulk_enrich_leads(
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"Errore bulk enrichment: {str(e)}"
        )
    finally:
        await service.close()
