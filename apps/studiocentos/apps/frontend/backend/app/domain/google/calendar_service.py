...
def get_admin_google_token(db: Session, admin_id: int) -> Optional[str]:
    """
    ...
    if settings_obj.token_expires_at:
        ...
        if settings_obj.token_expires_at.replace(tzinfo=timezone.utc) > now:
            # Token still valid
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            settings_obj.token_expires_at = datetime.now(timezone.utc).replace(
                microsecond=0
...
---
Gestito da StudioCentOS Backoffice
    """.strip()
