...
class GoogleDocsService:
    ...
    async def export_as_pdf(self, document_id: str) -> Optional[bytes]:
        """
    ...
    async def upload_file(
        ...
        boundary = "studiocentos_boundary"
...
async def generate_quote_pdf(
    ...
    replacements = {
        ...
        "{{COMPANY_NAME}}": "StudioCentOS",
        "{{COMPANY_ADDRESS}}": "Milano, Italia",
        "{{COMPANY_EMAIL}}": "info@studiocentos.it",
...
async def get_or_create_quotes_folder(
    ...
    for file in files:
        if file.get("name") == "StudioCentOS_Preventivi":
            return file.get("id")

    # Create folder
    folder_id = await service.create_folder("StudioCentOS_Preventivi")
    return folder_id
