...
from app.domain.auth.oauth_tokens import OAuthTokenService
...
async def initiate_google_connect(
    request: Request,
    force_consent: bool = Query(False, description="Force re-consent even if already connected"),
    ...
    If already connected with valid refresh token, skips consent prompt.
    Use force_consent=true to force re-authorization (e.g., to add new scopes).
    """
    # Check if admin already has valid refresh token
    ...
    has_valid_refresh_token = (
        google_settings
        and google_settings.refresh_token
    ...
    # - "consent" forces full consent screen (needed first time or when forcing)
    # - "select_account" just shows account picker (when already authorized)
    prompt = "select_account" if has_valid_refresh_token else "consent"
    ...
    logger.info(f"Initiating Google OAuth for admin {admin.id} (prompt={prompt}, force_consent={force_consent})")
...
async def get_business_service(
    ...
    if google_settings.token_expires_at and google_settings.token_expires_at < datetime.now(timezone.utc):
        raise HTTPException(
            status_code=status.HTTP_401_UNAUTHORIZED,
...
async def get_search_console_status(
    ...
    return {"connected": has_gsc, "site_url": "https://studiocentos.com"}
