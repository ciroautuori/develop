...
from app.core.security.encryption import audit_encryption, encryption_manager
...
class PCIDSSAuditor:
    ...
    def __init__(self):
        ...
        self.audit_date = datetime.now(timezone.utc)
...
async def generate_compliance_certificate() -> Dict[str, Any]:
    ...
    if audit_results["overall_compliance"]:
        return {
            "certificate_id": f"PCI-DSS-{int(datetime.now(timezone.utc).timestamp())}",
