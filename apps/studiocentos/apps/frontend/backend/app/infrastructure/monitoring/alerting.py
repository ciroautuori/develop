...
class AlertManager:
    ...
    async def _persist_alert(self, alert: Alert):
        ...
        try:
            ...
            from app.infrastructure.monitoring.models import AlertLog, AlertSeverityEnum, AlertCategoryEnum
...
class PaymentAlertMonitor:
    ...
    async def check_payment_failures(self, db: AsyncSession) -> Optional[Alert]:
        ...
        try:
            ...
            if total_payments > 10:  # Only alert if meaningful volume
                ...
                if failure_rate > self.alert_manager.alert_thresholds["payment_failure_rate"]:
                    return Alert(
                        id=f"payment-failure-{int(datetime.now(timezone.utc).timestamp())}",
    ...
    async def check_revenue_loss(self, db: AsyncSession) -> Optional[Alert]:
        ...
        try:
            ...
            if revenue_loss > self.alert_manager.alert_thresholds["payment_amount_loss"]:
                return Alert(
                    id=f"revenue-loss-{int(datetime.now(timezone.utc).timestamp())}",
...
class SecurityAlertMonitor:
    ...
    async def check_suspicious_login_attempts(self, db: AsyncSession) -> Optional[Alert]:
        """Check for suspicious login attempt patterns."""
        try:
            from sqlalchemy import select, func
            from app.domain.gdpr.models import DataAuditLog, AuditActionEnum
            ...
            if (
                ...
                return Alert(
                    id=f"suspicious-login-{int(datetime.now(timezone.utc).timestamp())}",
    ...
    async def check_rate_limit_violations(self, db: AsyncSession) -> Optional[Alert]:
        ...
        try:
            ...
            if (
                ...
                return Alert(
                    id=f"rate-limit-violation-{int(datetime.now(timezone.utc).timestamp())}",
...
class PerformanceAlertMonitor:
    ...
    async def check_api_response_times(self, db: AsyncSession) -> Optional[Alert]:
        ...
        try:
            ...
            if avg_response_time > self.alert_manager.alert_thresholds["api_response_time"]:
                return Alert(
                    id=f"api-performance-{int(datetime.now(timezone.utc).timestamp())}",
    ...
    async def check_error_rates(self, db: AsyncSession) -> Optional[Alert]:
        ...
        try:
            ...
            if error_rate > self.alert_manager.alert_thresholds["error_rate"]:
                return Alert(
                    id=f"error-rate-{int(datetime.now(timezone.utc).timestamp())}",
...
class UptimeAlertMonitor:
    ...
    async def check_service_availability(self, db: AsyncSession) -> Optional[Alert]:
        ...
        try:
            ...
            if current_uptime < self.alert_manager.alert_thresholds["availability_sla"]:
                return Alert(
                    id=f"uptime-sla-{int(datetime.now(timezone.utc).timestamp())}",
...
async def trigger_security_breach_alert(breach_type: str, details: str):
    ...
    alert = Alert(
        id=f"security-breach-{int(datetime.now(timezone.utc).timestamp())}",
