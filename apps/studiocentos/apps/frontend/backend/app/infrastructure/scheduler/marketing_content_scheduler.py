...
class MarketingContentScheduler:
    ...
    def _load_topics(self) -> List[str]:
        ...
        # Default topics per StudioCentOS
        return [
    ...
    async def trigger_now(self, custom_topic: Optional[str] = None) -> Dict:
        """
    ...
    async def trigger_just_in_time_post(
        self,
        planned_topic: str,
        planned_angle: str,
        ...
        """
        Just-in-Time Post Generation - Chiamato quando arriva il momento di pubblicare.
        
        Genera il contenuto SOLO al momento della pubblicazione, non in anticipo.
        Questo risparmia costi API e garantisce contenuti freschi.
        """
        logger.info("jit_post_trigger", topic=planned_topic, platform=platform)
        
        async with aiohttp.ClientSession() as session:
            url = f"{self.ai_url}/api/v1/marketing/post/trigger"
            headers = {
            ...
            try:
                async with session.post(url, json=payload, headers=headers, timeout=120) as resp:
                    if resp.status != 200:
                        ...
                        logger.error("jit_trigger_error", status=resp.status, error=error_text)
                        return {"success": False, "error": error_text}
                    
                    result = await resp.json()
                    logger.info("jit_post_generated",
                ...
                logger.error("jit_trigger_timeout")
                return {"success": False, "error": "Just-in-Time generation timeout"}
            except Exception as e:
                logger.error("jit_trigger_exception", error=str(e))
                return {"success": False, "error": str(e)}

    async def _generate_daily_content(self, custom_topic: Optional[str] = None) -> Dict:
        """
...
async def trigger_marketing_generation(custom_topic: Optional[str] = None):
...
async def trigger_jit_post(
    ...
    """
    Just-in-Time Post Trigger - Da chiamare al momento della pubblicazione.
    
    Esempio:
        result = await trigger_jit_post(
            planned_topic="AI Marketing Trends",
            planned_angle="5 modi per usare l'AI nel tuo business",
            platform="instagram"
        )
    """
    return await marketing_content_scheduler.trigger_just_in_time_post(
        planned_topic=planned_topic,
        planned_angle=planned_angle,
        platform=platform,
        sector=sector,
