"""Initial StudiocentOS schema - 5 domains
...
def upgrade() -> None:
    ...
    # Auth enums
    op.execute("CREATE TYPE userrole AS ENUM ('admin', 'user', 'customer', 'pro', 'trial', 'tester')")
    ...
    op.create_table(
        ...
        sa.Column('role', postgresql.ENUM('admin', 'user', 'customer', 'pro', 'trial', 'tester', name='userrole'), server_default='user', nullable=False),
    ...
    op.create_table(
        ...
        sa.Column('title', sa.String(200), nullable=False),
        sa.Column('description', sa.Text(), nullable=False),
        sa.Column('status', postgresql.ENUM('OPEN', 'IN_PROGRESS', 'WAITING_USER', 'RESOLVED', 'CLOSED', name='ticketstatus'), server_default='OPEN', nullable=False),
        sa.Column('priority', postgresql.ENUM('LOW', 'MEDIUM', 'HIGH', 'URGENT', name='ticketpriority'), server_default='MEDIUM', nullable=False),
