...
class GDPRComplianceManager:
    ...
    async def record_consent(self, db: AsyncSession, consent_data: ConsentModel) -> Dict[str, Any]:
        ...
        try:
            ...
            consent_data.granted_at = datetime.now(timezone.utc)
            ...
            if consent_data.purpose in [DataProcessingPurpose.LEGITIMATE_INTERESTS]:
                consent_data.expires_at = datetime.now(timezone.utc) + timedelta(days=730)

            # Save to database
            from app.domain.gdpr.models import UserConsent, DataProcessingPurposeEnum, ConsentStatusEnum
    ...
    async def export_user_data(
        ...
        try:
            ...
            from app.domain.portfolio.models import Experience, Education, Project, Skill, PortfolioLanguage, PortfolioProfile
            from app.domain.gdpr.models import UserConsent, DataAuditLog
    ...
    async def audit_data_processing(self, db: AsyncSession, user_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            from app.domain.gdpr.models import UserConsent, DataAuditLog, AuditActionEnum
    ...
    def validate_data_processing_lawfulness(
        self, purpose: DataProcessingPurpose, user_consent: Optional[ConsentModel] = None
    ) -> bool:
        ...
        if purpose == DataProcessingPurpose.CONSENT:
            return (
                ...
                and (not user_consent.expires_at or user_consent.expires_at > datetime.now(timezone.utc))
            )
    ...
    def _assess_legitimate_interest_balance(self, user_consent: Optional[ConsentModel] = None) -> bool:
...
async def process_data_export_request(
    ...
    export_request = DataExportRequest(
        user_id=user_id, requested_at=datetime.now(timezone.utc), format=format
    )
...
async def process_deletion_request(
    ...
    deletion_request = DataDeletionRequest(
        user_id=user_id, requested_at=datetime.now(timezone.utc), reason=reason, hard_delete=hard_delete
    )
