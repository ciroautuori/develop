...
class CSRFProtection:
    ...
    @staticmethod
    def generate_token(session_id: str, secret: Optional[str] = None) -> str:
        """Generate CSRF token using SECRET_KEY from settings."""
        _secret = secret or settings.SECRET_KEY
        data = f"{session_id}{_secret}{time.time()}"
        return hashlib.sha256(data.encode()).hexdigest()

    @staticmethod
    def validate_token(token: str, session_id: str, secret: Optional[str] = None) -> bool:
        """Validate CSRF token using SECRET_KEY from settings."""
