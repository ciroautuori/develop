...
async def google_login_redirect(db: Session = Depends(get_db)):
    ...
    Redirect a Google per autenticazione con studiocentos089@gmail.com.
...
async def google_login_callback(
    ...
    Autentica admin usando Google account (studiocentos089@gmail.com).
    ...
    try:
        ...
        # Verifica che sia l'email corretta
        if google_email != "studiocentos089@gmail.com":
...
def logout_admin(
    ...
    """
    # Log audit for logout event
    AdminAuthService._log_audit(
        db, current_admin.id, "admin_logout"
    )

    # Token invalidation handled client-side (JWT stateless)
    # Server-side blacklist not implemented for performance reasons
