...
class AgentOrchestrator:
    ...
    def handle_agent_failure(
        ...
        print(f"Agent {agent_type} failed: {error}")
    ...
    async def shutdown(self) -> None:
        ...
        if self._running_workflows:
            print(f"Waiting for {len(self._running_workflows)} workflows to complete...")
            ...
            while self._running_workflows:
                if (datetime.utcnow() - start_time).seconds > timeout:
                    print("Shutdown timeout - forcefully terminating")
                    break
