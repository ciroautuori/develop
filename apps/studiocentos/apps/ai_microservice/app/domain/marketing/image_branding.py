"""
Image Branding System - StudioCentOS Brand Overlay
...
# Brand Colors - STUDIOCENTOS REAL BRAND (NO BLU!)
BRAND_COLORS = {
...
class LogoPosition(str, Enum):
    ...
    BOTTOM_CENTER = "bottom_center"  # BEST - Centered at bottom
    CENTER = "center"
    AUTO = "auto" # New intelligent mode
...
class ImageBranding:
    ...
    def __init__(self):
        ...
        self._logo_white: Optional[Image.Image] = None
    ...
    def apply_branding(
        ...
        logo_position: str = "auto",  # Default to Intelligent Auto-Placement
        logo_size_ratio: float = 0.12,  # Smaller, more subtle
        add_footer: bool = False,  # DISABLED by default - ugly
        footer_text: str = "StudioCentOS",
        logo_url: Optional[str] = None,
        add_corner_accents: bool = False,  # DISABLED by default - ugly
    ) -> bytes:
        """
        ...
        Args:
            ...
            logo_url: Optional URL to custom logo (from Brand DNA). If provided, uses this instead of default.
        ...
        try:
            ...
            # Get logo to use - custom from URL or default
            logo_to_use = self._logo_white
            if logo_url:
                try:
                    import httpx
                    # Download custom logo from URL
                    response = httpx.get(logo_url, timeout=10.0)
                    if response.status_code == 200:
                        logo_to_use = Image.open(io.BytesIO(response.content)).convert("RGBA")
                        logger.info(f"Using custom logo from: {logo_url}")
                    else:
                        logger.warning(f"Failed to download logo from {logo_url}, using default")
                except Exception as e:
                    logger.warning(f"Error downloading custom logo: {e}, using default")
            ...
            if logo_to_use:
                ...
                if logo_position == "auto":
                    logo_position = self._detect_best_logo_position(image)
                    logger.info(f"Auto-detected best logo position: {logo_position}")
                    
                image = self._add_logo_dynamic(image, logo_to_use, logo_position, logo_size_ratio)

            # Add footer/watermark (DISABLED by default - ruins the image)
            if add_footer:
                image = self._add_footer(image, footer_text)

            # Add corner accents (DISABLED by default - ugly)
            if add_corner_accents:
    ...
    def _detect_best_logo_position(self, image: Image.Image) -> LogoPosition:
        """
        Analyze image to find the safest spot for the logo (least busy corner).
        Uses standard deviation of pixel brightness as a proxy for visual complexity.
        """
        try:
            ...
            # Define corners (width, height, name)
            # We look at 20% of the image size in each corner
            cw, ch = int(width * 0.25), int(height * 0.25)
            
            corners = {
                LogoPosition.TOP_LEFT: (0, 0, cw, ch),
                LogoPosition.TOP_RIGHT: (width - cw, 0, width, ch),
                LogoPosition.BOTTOM_LEFT: (0, height - ch, cw, height),
                LogoPosition.BOTTOM_RIGHT: (width - cw, height - ch, width, height)
            }
            
            variances = {}
            for pos, box in corners.items():
                region = gray.crop(box)
                # Calculate variance (standard deviation squared) of pixel values
                # Plain background = Low variance. Detailed subject = High variance.
                stat = region.getextrema()
                if stat:
                    # Approximated variance: (max - min) roughly indicates contrast/detail range
                    # For more precision, we need ImageStat, but let's stick to core PIL if possible
                    # Actually PIL has ImageStat
                    from PIL import ImageStat
                    stats = ImageStat.Stat(region)
                    variance = stats.stddev[0] # Standard deviation
                    variances[pos] = variance
                else:
                    variances[pos] = 999
            
            # Find corner with MINIMUM variance (cleanest area)
            best_pos = min(variances, key=variances.get)
            
            # Default to Bottom Right if all are similar or noisy, as it's standard
            if variances[best_pos] > 50: # Threshold for "everything is busy"
                return LogoPosition.BOTTOM_RIGHT
                
            return best_pos
            
        except Exception as e:
            logger.warning(f"Logo position detection failed: {e}")
            return LogoPosition.BOTTOM_RIGHT
    ...
    def _add_logo_dynamic(
        ...
        """
        Add logo overlay to image with DYNAMIC logo from parameter.

        This version accepts a logo image directly instead of using the internal one.
        Used for Brand DNA custom logos.
        """
        ...
        if position == LogoPosition.TOP_LEFT:
            ...
            pos = (padding, image.height - logo_height - padding)
        elif position == LogoPosition.BOTTOM_RIGHT:
            pos = (image.width - logo_width - padding, image.height - logo_height - padding)
        elif position == LogoPosition.BOTTOM_CENTER:
            pos = ((image.width - logo_width) // 2, image.height - logo_height - padding)  # Centered at bottom
        else:  # CENTER
            pos = ((image.width - logo_width) // 2, (image.height - logo_height) // 2)

        # Create a semi-transparent version
        if logo.mode == 'RGBA' and len(logo.split()) == 4:
            logo.putalpha(Image.eval(logo.split()[3], lambda x: int(x * 0.85)))
        else:
            # For logos without alpha channel, add one
            alpha = Image.new('L', logo.size, int(255 * 0.85))
            logo.putalpha(alpha)
    ...
    def _add_footer(self, image: Image.Image, text: str) -> Image.Image:
        """Add brand footer to image - STUDIOCENTOS GOLD & BLACK."""
    ...
    def create_social_template(
        ...
        # Add footer
        image = self._add_footer(image, "StudioCentOS")
    ...
    def create_business_dna_profile(
        ...
        for i, font_name in enumerate(fonts[:3]):  # Max 3 fonts
            draw.text((section_x, fonts_y + 50 + i * 35), f"Aa", fill=BRAND_COLORS["primary"], font=font_title)
