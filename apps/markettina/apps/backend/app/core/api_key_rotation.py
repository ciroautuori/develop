...
class APIKeyRotationService:
    ...
    def __init__(self, db: Session, redis_client: aioredis.Redis | None = None):
        self.db = db
    ...
    async def create_api_key(
        ...
        expires_at = datetime.now(UTC) + timedelta(days=expires_in_days or self.KEY_LIFETIME)
    ...
    async def rotate_api_key(self, key_id: str, user_id: int, keep_old_key_days: int = 7) -> dict:
        ...
        new_key = APIKey(
            ...
            expires_at=datetime.now(UTC) + timedelta(days=self.KEY_LIFETIME),
        ...
        old_key.expires_at = datetime.now(UTC) + timedelta(days=keep_old_key_days)
        old_key.last_rotated_at = datetime.now(UTC)
    ...
    async def validate_api_key(
        ...
        # Check expiration
        if db_key.expires_at and datetime.now(UTC) > db_key.expires_at:
    ...
    async def auto_rotate_expiring_keys(self):
        ...
        """
        warning_date = datetime.now(UTC) + timedelta(days=self.ROTATION_WARNING_DAYS)
