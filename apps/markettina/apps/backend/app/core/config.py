...
class Settings(BaseSettings):
    ...
    APP_NAME: str = os.getenv("APP_NAME", "MARKETTINA")
    PROJECT_NAME: str = os.getenv("PROJECT_NAME", "MARKETTINA Portfolio API")
    ...
    # Email settings
    EMAILS_FROM_EMAIL: str = os.getenv("EMAILS_FROM_EMAIL", "noreply@markettina.it")
    EMAILS_FROM_NAME: str = os.getenv("EMAILS_FROM_NAME", "MARKETTINA")
    ...
    # Admin settings
    ALLOWED_ADMIN_EMAILS: str = os.getenv(
        "ALLOWED_ADMIN_EMAILS",
        "markettina089@gmail.com,admin@markettina.it"
    )
    ...
    BASE_URL: str = os.getenv("BASE_URL", "https://markettina.it")
    ...
    SENDGRID_FROM_EMAIL: str = Field(default="", description="SendGrid verified sender email")
    SENDGRID_FROM_NAME: str = Field(default="MARKETTINA", description="SendGrid sender name")
    ...
    # =============================================================================
    GOOGLE_CREDENTIALS_JSON: str = Field(default="", description="Google Service Account JSON credentials")
    GOOGLE_SEARCH_CONSOLE_SITE: str = Field(default="", description="Google Search Console site URL")
    GA4_CREDENTIALS: str = Field(default="", description="GA4 Service Account credentials (can share with GSC)")
    ...
    def BACKEND_CORS_ORIGINS(self) -> list[str]:
        ...
        if self.ENVIRONMENT == "production":
            return [
                ...
                "https://markettina.it", "https://www.markettina.it", "https://app.markettina.it",
    ...
    CLEARBIT_API_KEY: str | None = Field(
        default=None, description="Clearbit API key for paid features"
    )
    BRANDFETCH_API_KEY: str | None = Field(
    ...
    # =============================================================================
    RAG_CHUNK_SIZE: int = Field(default=500, description="RAG document chunk size")
    RAG_CHUNK_OVERLAP: int = Field(default=50, description="RAG chunk overlap")
    RAG_SIMILARITY_THRESHOLD: float = Field(default=0.7, description="RAG min similarity score")
    CHROMADB_PERSIST_DIR: str = Field(default="/app/data/chroma", description="ChromaDB persistence directory")

    # Google API Key - for Gemini/AI features (resolved property)
    GOOGLE_API_KEY: str = Field(default="", description="Google API Key for AI features")

    @property
    def google_api_key_resolved(self) -> str:
        """Get Google API key with fallback chain."""
        return self.GOOGLE_AI_API_KEY or self.GOOGLE_API_KEY or os.getenv("GOOGLE_API_KEY", "")

    @property
    def huggingface_token_resolved(self) -> str:
        """Get HuggingFace token with fallback chain."""
        return self.HUGGINGFACE_API_KEY or os.getenv("HUGGINGFACE_TOKEN", "") or os.getenv("HUGGINGFACE_API_KEY", "")

    @property
    def groq_api_keys(self) -> list[str]:
        """Get list of GROQ API keys for rotation."""
        keys = []
...
def get_settings() -> Settings:
    """Get settings instance (dependency injection pattern for FastAPI)."""
    return settings
