...
class SocialMediaIntegration:
    ...
    async def _get_facebook_page_token(self) -> str | None:
            ...
            async with aiohttp.ClientSession() as session:
                ...
                async with session.get(url, params=params) as response:
                    if response.status == 200:
                        ...
                        if pages:
                            logger.warning(f"Page ID {self.facebook_page_id} not found, using first available page")
                            return pages[0].get("access_token")

                        logger.error("No Facebook pages found for this account")
                        return None
                    ...
                    try:
                        ...
                        logger.error(f"Failed to get Facebook page token: {error_msg} (status: {response.status})")
                    except:
                        logger.error(f"Failed to get Facebook page token: {error_text} (status: {response.status})")
                    return None
        except Exception as e:
            logger.exception(f"Exception in _get_facebook_page_token: {e!s}")
            return None
    ...
    async def publish_instagram(
        ...
        try:
            ...
            return {
                "status": "error",
                "message": f"Instagram integration exception: {e!s}"
            }
