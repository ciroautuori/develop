...
from app.domain.toolai.models import AITool, ToolAIPost, ToolAIPostStatus
...
class ToolAIScheduler:
    ...
    async def _generate_daily_post(self) -> dict:
        ...
        try:
            ...
            # Step 1: Fetch REAL tools da HuggingFace, GitHub, ArXiv
            logger.info("toolai_discovering_real_tools",
                       num_tools=self.num_tools,
                       categories=self.categories)

            scraper = ToolAIScraper(github_token=os.getenv("GITHUB_TOKEN"))
            # Fetch MORE tools (20) per poi selezionare i migliori
            all_discovered_tools = await scraper.discover_tools(
                num_tools=20,  # Fetch molti di più per avere scelta
                categories=self.categories if self.categories else None,
                sources=["github", "huggingface_models", "huggingface_papers", "arxiv"]  # GitHub PRIMA per repo trending
            )

            # Seleziona i TOP tool per trending score
            all_discovered_tools.sort(
                key=lambda x: (
                    x.get("trending_score", 0) * 2 +  # Peso doppio al trending score
                    x.get("stars", 0) +
                    x.get("downloads", 0) // 1000
                ),
                reverse=True
            )
