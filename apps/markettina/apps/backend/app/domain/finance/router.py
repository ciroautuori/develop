...
from .schemas import (
    ...
    PaginatedROIResponse,
    PaymentMethodStatsResponse,
    ROIFilters,
    ROIResponse,
    TaxDeductibilityResponse,
...
def get_expenses(
    ...
    supplier_name: str | None = Query(None, description="Filtra per fornitore"),
    date_from: date | None = Query(None, description="Data scadenza da"),
    date_to: date | None = Query(None, description="Data scadenza a"),
    amount_min: float | None = Query(None, ge=0, description="Importo minimo"),
    amount_max: float | None = Query(None, ge=0, description="Importo massimo"),
    is_overdue: bool | None = Query(None, description="Solo spese scadute"),
    tax_deductible: bool | None = Query(None, description="Solo spese deducibili"),
    project_id: int | None = Query(None, description="Filtra per progetto"),
    db: Session = Depends(get_db),
    admin: AdminUser = Depends(get_current_admin_user)
):
    """
...
def get_budgets(
    ...
    is_over_budget: bool | None = Query(None),
    db: Session = Depends(get_db),
    admin: AdminUser = Depends(get_current_admin_user)
):
...
def get_roi_trackings(
    ...
    is_profitable: bool | None = Query(None),
    db: Session = Depends(get_db),
    admin: AdminUser = Depends(get_current_admin_user)
):
...
def get_finance_alerts(
    page: int = Query(1, ge=1),
    page_size: int = Query(50, ge=1, le=100),
    status: str | None = Query(None, regex="^(active|dismissed|resolved|snoozed)$"),
