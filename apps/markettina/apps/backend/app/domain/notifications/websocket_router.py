...
async def websocket_notifications(
    ...
    Connessione:
    ```javascript
    const ws = new WebSocket(`wss://markettina.it/ws/notifications?token=${accessToken}`);
    ```
    """
    user_id = None
...
async def websocket_admin_notifications(
    ...
    Connessione:
    ```javascript
    const ws = new WebSocket(`wss://markettina.it/ws/admin/notifications?token=${adminToken}`);
    ```
    """
    try:
        # Valida token admin
