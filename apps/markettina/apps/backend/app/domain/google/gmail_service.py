...
class GmailService:
    ...
    def _create_message(
        ...
        if from_name:
            message["From"] = f"{from_name} <noreply@markettina.it>"
        ...
        <table width="100%" cellpadding="0" cellspacing="0" style="max-width: 600px; margin: 0 auto; background: white;">
            ...
            <tr>
                <td style="padding: 30px; background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); text-align: center;">
                    <h1 style="margin: 0; color: white; font-size: 28px;">MARKETTINA</h1>
...
async def send_quote_email(
    ...
    result = await service.send_email(
        to=client_email,
        subject=f"📄 Preventivo MARKETTINA: {project_title}",
        body_html=html,
        reply_to="info@markettina.it"
    )
