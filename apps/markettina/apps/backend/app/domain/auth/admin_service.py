...
from .admin_schemas import Admin2FASetupResponse, AdminLoginResponse
...
class AdminAuthService:
    ...
    def login(
        ...
        access_token = create_access_token(
            data={
                "sub": str(admin.id),
                "email": admin.email,
                "role": admin.role or "admin",  # Use role from DB
    ...
    def get_or_create_google_admin(
        ...
        Args:
            db: Database session
            google_email: Email Google (es. markettina089@gmail.com)
    ...
    def create_admin_session(db: Session, admin: AdminUser) -> AdminLoginResponse:
        ...
        access_token = create_access_token(
            data={"sub": str(admin.id), "type": "access", "role": admin.role or "admin", "jti": access_jti}
        )
        refresh_token = create_refresh_token(
            data={"sub": str(admin.id), "type": "refresh", "role": admin.role or "admin", "jti": refresh_jti}
