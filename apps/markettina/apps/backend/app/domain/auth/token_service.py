...
class TokenService:
    ...
    def create_admin_tokens(
        ...
        """Create tokens for admin user with role and scopes.
        ...
        """
        from app.core.constants.auth_constants import ROLE_PERMISSIONS

        # Get scopes based on role
        permissions = ROLE_PERMISSIONS.get(role, [])
        scopes = [p.value for p in permissions]

        # Create token payload with role and scopes
        token_data = {
            "sub": str(admin_id),
            "email": email,
            "role": role,
