...
class AICustomerSupport:
    ...
    def __init__(self):
        ...
        # MARKETTINA specific context
        self.system_context = """Sei un assistente AI per MARKETTINA, software house specializzata in sviluppo applicazioni enterprise a Salerno e Campania.

INFORMAZIONI CHIAVE SU markettina:
...
NON INVENTARE informazioni. Se non sei sicuro di qualcosa, dillo chiaramente e suggerisci il contatto diretto."""
    ...
    async def generate_response(
        ...
        for current_provider in providers_to_try:
            try:
                ...
                logger.warning(f"Provider {current_provider} failed: {e!s}")
                continue
    ...
    async def _groq_response(self, message: str, context: str | None) -> dict[str, Any]:
        ...
        try:
            ...
            logger.error(f"GROQ provider error: {e!s}")
            raise

    async def _gemini_response(self, message: str, context: str | None) -> dict[str, Any]:
        ...
        try:
            ...
            logger.error(f"Gemini provider error: {e!s}")
            raise

    async def _openrouter_response(self, message: str, context: str | None) -> dict[str, Any]:
        ...
        try:
            ...
            logger.error(f"OpenRouter provider error: {e!s}")
            raise

    async def _huggingface_response(self, message: str, context: str | None) -> dict[str, Any]:
        ...
        try:
            ...
            logger.error(f"HuggingFace provider error: {e!s}")
            raise

    async def _ollama_response(self, message: str, context: str | None) -> dict[str, Any]:
        ...
        try:
            ...
            logger.error(f"Ollama provider error: {e!s}")
            raise
    ...
    def _calculate_confidence(self, response: str, question: str) -> int:
        ...
        # Increase if response contains specific MARKETTINA terms
        markettina_terms = ["react", "fastapi", "app mobile", "e-commerce", "consulenza", "prenotazione", "scafati", "salerno", "campania"]
        matches = sum(1 for term in markettina_terms if term.lower() in response.lower())
