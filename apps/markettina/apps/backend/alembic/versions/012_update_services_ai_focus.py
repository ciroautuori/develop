...
2. Aggiorna i servizi per riflettere l'offerta AI di MARKETTINA
"""
...
def upgrade() -> None:
    ...
    if 'benefits' not in existing_columns:
        op.add_column('services', sa.Column('benefits', postgresql.JSONB(), server_default='[]', nullable=True))
    if 'is_active' not in existing_columns:
        op.add_column('services', sa.Column('is_active', sa.Boolean(), server_default='true', nullable=False))
    ...
    for service in SERVICES:
        # features is ARRAY(String), format as Postgres array literal: {"a","b"}
        features_items = [f'"{item}"' for item in service["features"]]
        features_pg = '{' + ','.join(features_items) + '}'
