"""
Post BRANDIZZATO con Sistema MARKETTINA
...
def apply_markettina_branding(image_bytes: bytes) -> bytes:
    """Applica branding MARKETTINA all'immagine."""
    print("\n🎨 Applicando branding MARKETTINA...")
...
async def upload_to_static(image_bytes: bytes, filename: str) -> str:
    ...
    # URL pubblico
    public_url = f"https://markettina.it/static/social/{filename}"
...
async def create_branded_post():
    ...
    print("=" * 80)
    print("🎨 POST BRANDIZZATO - MARKETTINA Auto-Publishing")
    ...
    # Step 2: Applica branding MARKETTINA
    branded_image = apply_markettina_branding(base_image)
...
MARKETTINA ti aiuta a integrare soluzioni AI reali nel tuo business:
...
👉 markettina.it"""
    ...
    payload = {
        "content": content,
        "title": "Post Brandizzato AI - MARKETTINA",
        "hashtags": ["#AI", "#Innovation", "#MARKETTINA", "#TechItalia"],
    ...
    try:
        async with httpx.AsyncClient(timeout=15.0) as client:
            ...
            if response.status_code == 200:
                ...
                print(f"✅ Immagine: BRANDIZZATA con logo MARKETTINA oro")
                ...
                print(f"   📌 Footer oro con nome MARKETTINA")
                print(f"   📌 Accenti oro negli angoli (design premium)")
                print(f"   📌 Watermark MARKETTINA semi-trasparente")
                print(f"   📌 Colori brand #D4AF37 (oro)")

                print(f"\n⏰ Controlla tra {publish_in_minutes} minuti:")
                print(f"   📘 Facebook: https://www.facebook.com/Markettina")
                print(f"   📸 Instagram: https://www.instagram.com/markettina")
                print("=" * 80)
