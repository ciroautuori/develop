...
export function ChatWidget() {
  ...
  const sendMessage = async () => {
    ...
    try {
      // Use demo endpoint for public access (no auth required)
      const endpoint = 'https://markettina.it/ai/api/v1/demo/ask'
  ...
  return (
    <>
      ...
      {isOpen && (
        <Card className={`fixed z-[9999] transition-all duration-300 shadow-2xl ${isMinimized
            ? 'bottom-4 right-4 sm:bottom-6 sm:right-6 w-72 sm:w-80 h-16'
            : 'inset-0 sm:inset-auto sm:bottom-6 sm:right-6 sm:w-[min(400px,calc(100vw-3rem))] sm:h-[600px] sm:max-h-[calc(100vh-3rem)] sm:rounded-xl rounded-none'
          }`}>
          ...
          {!isMinimized && (
            <CardContent className="flex flex-col h-[calc(100vh-60px)] sm:h-[520px] p-0">
              {/* Messages - Improved Design */}
