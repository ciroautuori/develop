...
export default function SocialPublisherPro() {
  ...
  const generateAIImages = async () => {
    ...
    try {
      ...
      for (const platformId of selectedPlatforms) {
        ...
        const res = await fetch('/api/v1/admin/copilot/image/generate', {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
            Authorization: `Bearer ${localStorage.getItem('admin_token')}`,
  ...
  const generateFromTemplate = async (templateValue: string) => {
    ...
    try {
      ...
      const targetPlatform = selectedPlatforms[0] || 'linkedin';
      const res = await fetch('/api/v1/admin/copilot/content/generate', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          Authorization: `Bearer ${localStorage.getItem('admin_token')}`,
        ...
        body: JSON.stringify({
          ...
          brand_context: brandContext, // 🧬 Inject Brand DNA
        }),
      });
