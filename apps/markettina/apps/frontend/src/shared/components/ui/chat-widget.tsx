...
export function ChatWidget({ className }: ChatWidgetProps) {
  ...
  const [messages, setMessages] = useState<Message[]>([
    {
      id: "welcome",
      content: "Ciao! Sono l'assistente AI di MARKETTINA. Come posso aiutarti?",
      isUser: false,
      timestamp: new Date()
    }
  ])
  ...
  const sendMessage = async () => {
    ...
    try {
      // Chiamata AI reale
      const response = await fetch('/api/v1/demo/ask', {
        method: 'POST',
        headers: {
      ...
      const errorMessage: Message = {
        id: (Date.now() + 1).toString(),
        content: "Mi dispiace, sto avendo difficoltà tecniche. Per assistenza immediata contatta info@markettina.it o chiama il nostro ufficio di Salerno.",
        isUser: false,
        timestamp: new Date()
      }
        ...
        <div className={cn(
          ...
          <div className="flex items-center justify-between px-4 py-3 border-b border-border bg-card">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h3 className="font-semibold text-foreground text-sm">MARKETTINA AI</h3>
